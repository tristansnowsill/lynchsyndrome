...
from lynchsyndrome.experiments.common.results import CancerOutcomes, ParameterReport, PatientLevelCostReport, PatientLevelLifeYearsReport, PatientLevelQALYsReport, ResourceUseReport, SurvivalType
...
# ):
    ...
    annual_gynae_surveillance = AnnualGynaecologicalSurveillance(25, 75)
    offer_hbso_nosurv = OfferRiskReducingHBSO(35, False)
    offer_hbso_surv = OfferRiskReducingHBSO(35, True)

    # Combined pathways which will compete against each other
    no_gynae_risk_reduction = Pathway(
        { PathwayPoint.COLORECTAL_SURVEILLANCE: { biennial_colonoscopy } },
        'No gynaecological cancer risk reduction'
    )
    rrgs_only = Pathway(
        {
            PathwayPoint.COLORECTAL_SURVEILLANCE: { biennial_colonoscopy },
            PathwayPoint.GYNAECOLOGICAL_RISK_REDUCING_SURGERY: { offer_hbso_nosurv }
        },
        'Risk-reducing gynaecological surgery offered from age 35'
    )
    surveillance_only = Pathway(
        {
            PathwayPoint.COLORECTAL_SURVEILLANCE: { biennial_colonoscopy },
            PathwayPoint.GYNAECOLOGICAL_SURVEILLANCE: { annual_gynae_surveillance }
        },
        'Surveillance from age 25'
    )
    surveillance_and_rrgs = Pathway(
        {
            PathwayPoint.COLORECTAL_SURVEILLANCE: { biennial_colonoscopy },
            PathwayPoint.GYNAECOLOGICAL_RISK_REDUCING_SURGERY: { offer_hbso_surv },
            PathwayPoint.GYNAECOLOGICAL_SURVEILLANCE: { annual_gynae_surveillance }
        },
        'Surveillance from age 25 and RRGS offered from age 35'
    )

    # Register competing options with experiment
    experiment.add_competing_option(no_gynae_risk_reduction)
    experiment.add_competing_option(rrgs_only)
    experiment.add_competing_option(surveillance_only)
    experiment.add_competing_option(surveillance_and_rrgs)
