...
model {
    ...
    // Adenoma
    theta_cons ~ normal(-4, 3);
    theta_age  ~ lognormal(-6, 1);
    rho0  ~ beta(1, 20);
    rho1  ~ normal(40, 5);
    rho2  ~ lognormal(3.0, 0.25);
    eta   ~ beta(1, 20);
    phi   ~ beta(1, 20);
    psi   ~ beta(1, 20);
    kappa ~ beta(1, 20);
    ...
    // CRC
    rate_LS_progression_AB ~ lognormal(0.5, 0.5);
    rate_LS_progression_BC ~ lognormal(0.5, 0.5);
    rate_LS_progression_CD ~ lognormal(0.5, 0.5);

    // Study                     MLH1 MSH2 MSH6 PMS2
    // -----                     ---- ---- ---- ----
    // Ahadova et al. (2018)        9    8    6    1
    // Akiyama et al. (2000)        .    .    .    .
    // Aldecoa et al. (2016)        .    .    .    .
    // Chang et al. (2018)          2    5    5    2
    // De Jong et al. (2004)       11   17    .    .
    // Ferreira et al. (2008)       5    2    .    .
    // Giuffre et al. (2005)        .    .    .    .
    // Halvarsson et al. (2005)    20    7    .    .
    // Iino et al. (2000)           .    .    .    .
    // Loukola et al. (1999)        5    .    .    .
    // Meijer et al. (2009)        25   12    4    .
    // Muller et al. (2006)         .    .    .    .
    // Pedroni et al. (2001)        3    2    .    .
    // Pino et al. (2009)           6    7    2    .
    // TODO - MORE OF THESE
    Dabir_genotype ~ dirichlet([89,60,17]);
    ...
    // print("n_MSS_under60:", n_MSS_under60);
    // print("n_MSS_over60:", n_MSS_over60);
    // print("n_MSS_lowrisk:", n_MSS_lowrisk);
    // print("n_MSS_highrisk:", n_MSS_highrisk);
    // print("n_MSI_under60:", n_MSI_under60);
    // print("n_MSI_over60:", n_MSI_over60);
    // print("n_MSI_lowrisk:", n_MSI_lowrisk);
    // print("n_MSI_highrisk:", n_MSI_highrisk);
    ...
    real pred_log_risk_ratio_under60 = log(odds_under60) + log1p(odds_over60) - log1p(odds_under60) - log(odds_over60);
    ...
    real pred_log_risk_ratio_lowrisk = log(odds_lowrisk) + log1p(odds_highrisk) - log1p(odds_lowrisk) - log(odds_highrisk);

    // print("odds_MSI_MLH1:", odds_MSI_MLH1);
    // print("odds_MSI_MSH2:", odds_MSI_MSH2);
    // print("odds_MSI_MSH6:", odds_MSI_MSH6);
    // print("odds_under60:", odds_under60);
    // print("odds_over60:", odds_over60);
    // print("pred_log_risk_ratio_under60:", pred_log_risk_ratio_under60);
    // print("odds_lowrisk:", odds_lowrisk);
    // print("odds_highrisk:", odds_highrisk);
    // print("pred_log_risk_ratio_lowrisk:", pred_log_risk_ratio_lowrisk);
            ...
            for (i in 1:3) {
                ...
                for (age in 1:70) {
                    if (age + 1 < age_index_colonoscopy) {
                        ...
                        // There is a colonoscopy every year but some are missed...
    ...
    // Dabir et al. (2020)
    Dabir_MSI_MLH1 ~ binomial(Dabir_N_MLH1, odds_MSI_MLH1 / (1 + odds_MSI_MLH1));
    Dabir_MSI_MSH2 ~ binomial(Dabir_N_MSH2, odds_MSI_MSH2 / (1 + odds_MSI_MSH2));
    Dabir_MSI_MSH6 ~ binomial(Dabir_N_MSH6, odds_MSI_MSH6 / (1 + odds_MSI_MSH6));
    log_risk_ratio_under60 ~ normal(pred_log_risk_ratio_under60, se_log_risk_ratio_under60);
    log_risk_ratio_lowrisk ~ normal(pred_log_risk_ratio_lowrisk, se_log_risk_ratio_lowrisk);

    // PLSD (Dominguez-Valentin et al. 2020)
    PLSD_crc_MLH1_male ~ poisson(-log1m(p_PLSD_MLH1_male ./ S_PLSD_MLH1_male) .* PLSD_exposure_MLH1_male / 5.0);
    PLSD_crc_MLH1_female ~ poisson(-log1m(p_PLSD_MLH1_female ./ S_PLSD_MLH1_female) .* PLSD_exposure_MLH1_female / 5.0);
    PLSD_crc_MSH2_male ~ poisson(-log1m(p_PLSD_MSH2_male ./ S_PLSD_MSH2_male) .* PLSD_exposure_MSH2_male / 5.0);
    PLSD_crc_MSH2_female ~ poisson(-log1m(p_PLSD_MSH2_female ./ S_PLSD_MSH2_female) .* PLSD_exposure_MSH2_female / 5.0);
    PLSD_crc_MSH6_male ~ poisson(-log1m(p_PLSD_MSH6_male ./ S_PLSD_MSH6_male) .* PLSD_exposure_MSH6_male / 5.0);
    PLSD_crc_MSH6_female ~ poisson(-log1m(p_PLSD_MSH6_female ./ S_PLSD_MSH6_female) .* PLSD_exposure_MSH6_female / 5.0);
    PLSD_crc_PMS2 ~ poisson(-log1m(p_PLSD_PMS2 ./ S_PLSD_PMS2) .* PLSD_exposure_PMS2 / 5.0);
    
    // Engel et al. (2018)
    Germany_index_findings ~ multinomial(p_Germany_index);
    Netherlands_index_findings ~ multinomial(p_Netherlands_index);
    Finland_index_findings ~ multinomial(p_Finland_index);

    Germany_fu_adenomas ~ poisson(Germany_fu_colonoscopies * exp_Germany_fu_adenoma);
    Netherlands_fu_adenomas ~ poisson(Netherlands_fu_colonoscopies * exp_Netherlands_fu_adenoma);
    Finland_fu_adenomas ~ poisson(Finland_fu_colonoscopies * exp_Finland_fu_adenoma);

    Germany_fu_crc ~ multinomial(p_Germany_crc / sum(p_Germany_crc));
    Netherlands_fu_crc ~ multinomial(p_Netherlands_crc / sum(p_Netherlands_crc));
    Finland_fu_crc ~ multinomial(p_Finland_crc / sum(p_Finland_crc));
}
